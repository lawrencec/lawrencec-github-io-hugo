...
YAHOO.EU.ModuleTabs.prototype.onTabSwapped = function(type,aArgs) { 
		//YAHOO.util.DDM.swapNode(aArgs[0].source, aArgs[0].target);
		
		//iterate thru aTabs using getTab to swap items in aTabs
		...
		for(var i=0,oTab;oTab = this.aTabs[i];i++) {
			//window.console.log(oTab.getTab().id +  " " + sSourceTabId)
		...
		*/
		...
		if (this.aEvents["TabSwapped"]) {
			this.aEvents["TabSwapped"].fire(aArgs[0].source,aArgs[0].target);
		}	
